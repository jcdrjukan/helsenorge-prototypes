...
**Empty vs. populated state:** these are two separate Figma frames, so each gets its own URL via an internal hash route rather than an in-page toggle — `#populert` (default) and `#tom`, handled inside `SykdomKritiskInfo` the same way `psykisk-helse` manages its own sub-view hashes (`window.location.hash` + a `hashchange` listener, independent of App.tsx's top-level prototype routing). On the dedicated site: `sykdomogkritiskinfo.netlify.app/#tom` for the empty state.
...
**Resepter view (2026-07-17 rebuild):** replaced the earlier simplified card list with a faithful recreation of a real reference screenshot the user shared (Figma file `UAhljteF5I4yI9lwrpxta7`, node `3396:937`). Now includes: a "Forny resept" primary button, an expandable "Trenger du hjelp til å hente medisiner?" disclosure, a `Tabs` split ("Resepter" / "Resepthistorikk" — the latter is a stub, no historical data modeled), a "Vis kun" filter panel with two checkboxes ("Aktive e-resepter", "Resepter med refusjon" — both real filters now, `harRefusjon` added per prescription), a decorative "Sorter etter" `Select`, and richer `Panel` cards (`StatusDot` status + subtext, Virkestoff, FAST/VED BEHOV + indication, a divider, dispensed-brand link with "(tilsvarer X)", "Tilsvarer [shortname]", and per-item detail — lege/reseptnummer/gyldig-til — behind the native `Panel.ExpandedContent` "Se detaljer" toggle instead of shown inline). Ends with a "Vis utskriftsvennlig side" button and a "Personvern for Resepter" link list, both matching the reference.

Per explicit instruction, the six medications shown are the *same* ones as the PLL catalog (Lamotrigin, Rivaroksaban, Metoprolol, Kandesartan, Paracetamol, Valporinsyre) rather than the reference screenshot's own drug names (Lipitor, Norvasc, Aerius, Shingrix, TicoVac, etc.) — vaccines were dropped entirely since PLL has no vaccine data to harmonize against. A new `pllInfoFor()` helper in `data.ts` looks up indikasjon/dosering/sist-utlevert directly from the PLL entries so Resepter and PLL literally share one source of truth instead of two copies that happen to agree.
