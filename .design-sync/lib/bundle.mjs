// forked from design-sync lib/bundle.mjs — stub .scss/.module.scss imports so
// the @helsenorge/designsystem-react component entries (which ship uncompiled
// .scss imports) can be bundled. Styles come from _ds_bundle.css instead.
...
export async function bundleToIife({ entry, globalName, nodePaths, out, tsconfig, externals = [] }) {
  ...
  const scssStub = {
    name: 'scss-stub',
    setup(build) {
      build.onLoad({ filter: /\.module\.scss$/ }, () => ({ contents: 'export default new Proxy({}, { get: (_, p) => p })', loader: 'js' }));
      build.onLoad({ filter: /\.scss$/ }, () => ({ contents: '', loader: 'css' }));
    },
  };
  const plugins = [scssStub, reactShim];
